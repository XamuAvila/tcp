...
const createAircraft = (client) => {
  const now = new Date();
  client.write(`<event how="m-g-i-g-o" stale="${addSeconds(
    now,
    10
  ).toISOString()}" start="${now.toISOString()}"
          ...
          <detail action="${PUSH_AIRCRAFT}">
              <aircraftCreate>
                  ...
                  <platformDesignation>AR5-Tester REPMUSAMU</platformDesignation>
                  <platformUuid>G-TEKR-Tester REPMUSAMU</platformUuid>
                  ${SENSOR_CONFIG}
